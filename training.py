...
class Train():
    ...
    def train_epoch(self):
        ...
        (hidden, cell) = (torch.zeros(self.model.lstm.num_layers, self.batch_size, self.model.lstm.hidden_size).to(self.device),
                          torch.zeros(self.model.lstm.num_layers, self.batch_size, self.model.lstm.hidden_size).to(self.device))
        ...
        for n in range(num_train_batches):
            ...
            output, (hidden, cell) = self.model(batch[:,:-1], hidden, cell) #output.shape = (batch_size, vocab_size)
            target = batch[:,-1] #target.shape = (batch_size)
            #this is correct according to https://pytorch.org/docs/stable/generated/torch.nn.CrossEntropyLoss.html first example                
    ...
    def validate_epoch(self):
        ...
        with torch.no_grad(): #since in validation phase there is no backprop and weight updates
            ...
            for n in range(num_val_batches):
                ...
                output, (hidden, cell) = self.model(batch[:,:-1], hidden, cell)
                target = batch[:,-1]
                loss = self.loss_fct(output, target)
    ...
    def early_stopping_check(self):
        ...
        if (epoch_val_perplexity <= self.threshold_val_loss):
            ...
            if (current_epoch>=2):
                os.remove(f'model_epoch{self.early_stopping_checkpoints[-2]}.pt')      
